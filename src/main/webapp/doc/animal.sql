...
CREATE TABLE `d_report` (
  ...
  COMMENT '数据库表名',
  `report_template_name`  VARCHAR(128)     DEFAULT ''
  COMMENT '填报模板名称',
  `summary_template_name` VARCHAR(128)     DEFAULT ''
  COMMENT '汇总模板名称',
  `report_type`           SMALLINT         DEFAULT 0
  COMMENT '报表类型：1：月报 2：周报 3：统计 4：其他',
  `report_category`       SMALLINT         DEFAULT 0
  COMMENT '栏目 1：月度免疫（月） 2：春秋防周报（周） 3：春秋防数据汇总（半年） 4：物资储备（季） 5:应急管理（实时）',
...
DROP TABLE IF EXISTS `d_report_cycle`;
CREATE TABLE `d_report_cycle` (
...
CREATE TABLE `d_report_fill` (
  ...
  COMMENT '填报类型 1:常规 2：补填报',
  `report_cycle`  VARCHAR(32)         DEFAULT ''
  COMMENT '报表周期',
  `report_time`   TIMESTAMP NULL      DEFAULT NULL
  COMMENT '上报时间',
  `province_code` VARCHAR(32)         DEFAULT ''
  COMMENT '省代码',
